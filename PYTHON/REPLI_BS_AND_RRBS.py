    ...
    with open(input_file_path, "r") as input_file:
        ...
        while line:
            ...
            if line_counter >= start_line:
                ...
                try:
                    chr_i, start, end, fraction, percentage = line_contents
                    methy_reads, tot_reads = re.split('/', fraction.strip('\''))
                    ltws.append('\t'.join([chr_i, start, end, methy_reads, tot_reads, percentage]) )
                except ValueError, e:
                    pass
            line = input_file.readline()
...
def convert_file_format_pipeline():
    base_dir = '/Users/Ren/PycharmProjects/prediction_by_k/DATA/Repli_RRBS_data/'
    for file in [f for f in os.listdir(base_dir)]:
        if file.endswith('.bed'):
            filename_without_extension = os.path.splitext(file)[0]
            read_bed_file_and_convert(os.path.join(base_dir, file),
                                      os.path.join(base_dir, filename_without_extension + '_converted.bed'))
